...
if ($user_data['user_role'] != PRINCIPAL_USER) {
...
if (isset($_GET['statusOpen']) || isset($_GET['statusClose'])) {
    $status_open = (int)sanitize($_GET['statusOpen']);
    $status_close = (int)sanitize($_GET['statusClose']);

    if (isset($_GET['statusOpen'])) {
        $db->query("UPDATE `acad_year` SET `close_acad_year` = 0, `re_open_by` = '{$auth_user_name}' WHERE `acad_year_id` = '{$status_open}'");
    } else {
        $db->query("UPDATE `acad_year` SET `close_acad_year` = 1, `closed_by` = '{$auth_user_name}' WHERE `acad_year_id` = '{$status_close}'");
    }
    redirect('enroll-status.php');
}
...
<div class="container-fluid mt-3">
    <div class="grid">
        <div class="col">
            <div class="container">
                <h2 class="text-secondary">Academic Calender</h2>
                <p>Open and close an academic year. Admission officers will have wait for academic year opening</p>
            </div>
        </div>
        <section>
            <div class="container">
                            ...
                            <div class="card ms-2 mx-2">
                                ...
                                <div class="row mt-3 ms-1 mx1">
                                    <div class="col-md-12">
                                        ...
                                        ?>
                                        <form class="row" action="enroll-status.php?academic_year=1" method="post">
                                            <div class="col">
                                                <div class="mb-2">
                                            ...
                                            <?php while ($input_by_res = mysqli_fetch_assoc($input_by_query)) : ?>
                                                <tr>
                                                    ...
                                                    <?php if ($acad_year_res['close_acad_year'] == 0) : ?>
                                                        <td><a href="enroll-status.php?statusClose=<?= $acad_year_res['acad_year_id'] ?>" class="btn btn-danger btn-sm">Close</a></td>
                                                    <?php else : ?>
                                                        <td><a href="enroll-status.php?statusOpen=<?= $acad_year_res['acad_year_id'] ?>" class="btn btn-outline-dark btn-sm">Reopen</a></td>
                                                    <?php endif; ?>
                                                </tr>
                                            <?php endwhile; ?>
                                        <?php endwhile; ?>
                            ...
                            <div class="card mx-2">
                                ...
                                <nav aria-label="Page navigation example p-2">
                                    <ul class="pagination justify-content-end">
                                        <li class="page-item">
                                            <?php if ($page >= 2) : ?>
                                                <a class="page-link" href="enroll-status.php?page=<?= ($page - 1) ?>" tabindex="-1">Previous</a>
                                            <?php endif; ?>
                                        </li>
                                        <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                                            <?php if ($i == $page) : ?>
                                                <li class="page-item"><a class="page-link" href="enroll-status.php?page=<?= $i ?>"><?= $i ?></a></li>
                                            <?php else : ?>
                                                <li class="page-item"><a class="page-link" href="enroll-status.php?page=<?= $i ?>"><?= $i ?></a></li>
                                            <?php endif; ?>
                                        <?php endfor; ?>
                                        <li class="page-item">
                                            <?php if ($page < $total_pages) : ?>
                                                <a class="page-link" href="enroll-status.php?page=<?= ($page + 1) ?>">Next</a>
                                            <?php endif; ?>
                                        </li>
                                    </ul>
                                </nav>

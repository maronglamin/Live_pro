...
$inputs = $db->query("SELECT * FROM `enroll_student` WHERE `enroll_done` = 1 ORDER BY `stud_id` DESC LIMIT $start_from, $per_page_record");

if(isset($_GET['app'])) {
    $stud_id = (int)sanitize($_GET['app']);

    $db->query("UPDATE `enroll_student` SET `auth_enroll` = '1' WHERE `stud_id` = '{$stud_id}'");
    redirect('dashboard.php');
}
?>

<h2 class="text-secondary">Enrolment History</h2> 
<p>Approve Enrolled students inputted by the Admission officers</p>
<div class="card">
<div class="container">
<?php if(!isset($_GET['view'])):?>
    <h3 class="text-center text-secondary">Approve Admission Inputs</h3>
    ...
    <thead>
        <tr>
            ...
            <th scope="col">Inputted by</th>
            <th scope="col">Date Inputted</th>
            <th scope="col"></th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                <?=$input['stud_inputted_by']?>
                </td>
                <td><?=human_date($input['stud_inputted_at'])?></td>
                <td>
                <a href="admission/file-profile.php?complete=<?=$input['stud_id']?>" class="btn btn-sm btn-outline-dark">view</a>
                    <?php if ($input['auth_enroll'] == 1):?>
                        <span class="text-secondary">Approved</span>
                    <?php else:?>
                    <a href="dashboard.php?app=<?=$input['stud_id']?>" class="btn btn-sm btn-outline-dark">Approve Record</a>
                    <?php endif;?>
                </td>
            </tr>
            <?php endwhile;?>
        ...
        <?php
            $enroll_stud_query = $db->query("SELECT COUNT(*) FROM `enroll_student` WHERE `enroll_done` = 1 ORDER BY `stud_id` DESC");
